...
public class PlayerEat : MonoBehaviour
{
    ...
    public GameObject[] enemyArchers;
    ...
    public void UpdateEnemy()
    {
        ...
        enemyArchers = GameObject.FindGameObjectsWithTag("EnemyArcher");  // New line to update EnemyArcher objects
    }
    ...
    public void CheckEnemy()
    {
        ...
        CheckGameObject(enemyArchers);  // New line to check EnemyArchers
    }
    ...
    //     }
    // }
    public void CheckGameObject(GameObject[] objects)
    {
        foreach (GameObject obj in objects)
        {
            if (obj == null) continue;
            Transform m = obj.transform;
            float playerRadius = transform.localScale.x / 2;
            float objectRadius = m.localScale.x / 2;

            if (Vector2.Distance(transform.position, m.position) <= playerRadius + objectRadius)
            {
                if (obj.CompareTag("Food") || obj.CompareTag("Ammo"))
                {
                    HandleFoodOrAmmoCollision(obj);
                }
                else if (obj.CompareTag("Enemy") || obj.CompareTag("EnemyArcher"))  // Added EnemyArcher check here
                {
                    HandleEnemyCollision(obj);
                }
                else if (obj.CompareTag("Boss"))
                {
                    HandleBossCollision(obj);
                }
            }
        }
    }

    void HandleFoodOrAmmoCollision(GameObject obj)
    {
        Destroy(obj);
        RemoveObject(obj);
        ...
        {
            PlayerGrow();
            ms.RemoveObject(obj, ms.created_food);
            Destroy(obj);
        }
        else if (obj.CompareTag("Ammo"))
        {
            ms.RemoveObject(obj, ms.created_ammos);
            bulletCount += 1;
            UpdateBulletText();
        }
    }
    ...
    void HandleEnemyCollision(GameObject enemy)
    {
        if (transform.localScale.x > enemy.transform.localScale.x)
        {
            ...
            Destroy(enemy); // Destroy immediately upon eating
            ms.RemoveObject(enemy, ms.created_enemies);
    ...
    void HandleBossCollision(GameObject bossObject)
    {
        if (transform.localScale.x > bossObject.transform.localScale.x)
        {
            PlayerGrow();
            Destroy(bossObject);
            WinGame();
    ...
    {
        List<GameObject> objectList = new List<GameObject>(food);
