...
public class PlayerEat : MonoBehaviour
{
    ...
    public bool archerActivated = false; // Add this line
    ...
    public void UpdateEnemy()
    {
        ...
        // the scene starts 
        // Find enemies with tag "Enemy"
        GameObject[] enemiesWithTag = GameObject.FindGameObjectsWithTag("Enemy");
        // Find EnemyArchers with tag "EnemyArcher"
        GameObject[] archersWithTag = GameObject.FindGameObjectsWithTag("EnemyArcher");
        // Combine both arrays into one
        int totalEnemies = enemiesWithTag.Length + archersWithTag.Length;
        ...
        foreach (GameObject archer in archersWithTag)
        {
            enemies[index++] = archer;
        }
    }
    ...
    public void CheckGameObject(GameObject[] Object)
    {
        for (int i = 0; i < Object.Length; i++)
        {
            ...
            if (Vector2.Distance(transform.position, m.position) <= playerRadius + objectRadius)
            {
                ...
                else if (m.gameObject.CompareTag("Enemy") || m.gameObject.CompareTag("EnemyArcher"))
                {
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        else if (collision.gameObject.CompareTag("EnemyArcher"))
        {
            ...
            if (transform.localScale.x > archerTransform.localScale.x)
            {
                PlayerGrow();
                PlayerGrow();
                Destroy(collision.gameObject);
                ms.RemoveObject(collision.gameObject, ms.created_enemies);
    ...
    public void GainExperience(int xp)
    {
        ...
        if (experience >= 10)
        {
            ActivateEnemyArcher(); // Activate EnemyArcher when EXP reaches 10
        }
    ...
    public void ActivateEnemyArcher()
    {
        archerActivated = true; // Ensure archers are only activated once
        Debug.Log("Activating Enemy Archers!!!!!!");

        // Start spawning EnemyArchers via ObjectGenerator
        if (ms != null)
        {
            ms.StartSpawningEnemyArchers();
        }
        else
        {
            Debug.LogError("ObjectGenerator reference is missing in PlayerEat script.");
        }
    }
